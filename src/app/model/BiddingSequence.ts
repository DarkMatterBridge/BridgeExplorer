...
export class BiddingSequence {
  ...
  // importBnodeSequence(bNodeSequence: BNodeSequence): void {
  //   // this.bids = bNodeSequence.bids.slice(1);
  //   this.bids = [];
  //   alert(bNodeSequence.bids.length + ' ' + bNodeSequence.bnc.length);
  ...
  determineLastContractBid(): void {
    const lastBid = this.bids.slice().reverse().find(b => b.isContractBid());
